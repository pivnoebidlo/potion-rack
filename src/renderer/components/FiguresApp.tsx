    ...
    useEffect(() => {
        ...
        const handlePaste = async (e: ClipboardEvent) => {
            ...
            for (const item of Array.from(items)) {
                if (item.type.startsWith('image/')) { e.preventDefault(); const file = item.getAsFile(); if (file) { try { const compressed = await compressImage(file, 240, 240, 0.7); const imageMarkdown = `![${file.name}](${compressed})`; const { from } = view.state.selection.main; view.dispatch({ changes: { from, insert: imageMarkdown + '\n' } }); onChange(view.state.doc.toString()); } catch (err) { console.error('Failed to insert image:', err); } } return; }
            }
        };
    ...
    return (
        <div className={styles.editorRoot} ref={editorContainerRef}>
            <div className={styles.editorToolbar}>
                <button onClick={() => setPreview(!preview)} className={styles.editorBtn}>{preview ? '✏️ ' + $t.edit : '👁 ' + $t.preview}</button>
                <button onClick={onSave} className={`${styles.editorBtn} ${styles.editorBtnPrimary}`}>💾 {$t.saveContent}</button>
            </div>
            {preview ? <div className={styles.previewPane} dangerouslySetInnerHTML={{ __html: marked(content, { breaks: true }) as string }} /> : (
                <div className={styles.editorPane}>
                    <CodeMirror value={content} onChange={onChange} onCreateEditor={(view) => { editorViewRef.current = view; }} extensions={[keymap.of([...defaultKeymap, ...historyKeymap]), history(), markdown({ base: markdownLanguage, codeLanguages: languages }), oneDark, EditorView.lineWrapping, imagePreviewPlugin, EditorView.theme({ '.cm-gutters': { display: 'none' }, '.cm-activeLineGutter': { display: 'none' } })]} height="100%" style={{ height: '100%' }} />
                </div>
            )}
        </div>
    );
...
export default function FiguresApp() {
    ...
    const handleSave = async () => { if (!selected) return; await updateFigureAPI(selected.id, { content: editorContent }); await loadFigures(); };
    ...
    return (
        <div className={styles.root}>
            ...
            <div className={`${styles.leftPanel} ${leftPanelCollapsed ? styles.leftPanelCollapsed : styles.leftPanelExpanded}`}>
                ...
                <div style={{ opacity: leftPanelCollapsed ? 0 : 1, pointerEvents: leftPanelCollapsed ? 'none' : 'auto', transition: 'opacity 0.3s', display: 'flex', flexDirection: 'column', flex: 1, overflow: 'hidden' }}>
                    <div className={styles.searchBox}>
                        <input type="text" placeholder={$t.search + '...'} value={search} onChange={e => setSearch(e.target.value)} className={styles.searchInput} />
                        <div className={styles.filterRow}>
                            <select value={statusFilter} onChange={e => setStatusFilter(e.target.value)} className={styles.filterSelect}>
                                <option value="all">{$t.all}</option><option value="draft">{$t.draft}</option><option value="in-progress">{$t.inProgress}</option><option value="completed">{$t.completed}</option>
                            </select>
                            <button onClick={() => setViewMode(viewMode === 'grid' ? 'list' : 'grid')} className={styles.viewToggleBtn}>{viewMode === 'grid' ? '☰' : '⊞'}</button>
                        </div>
                    </div>
                    ...
                    <div className={styles.figureList}>
                        {viewMode === 'grid' ? filtered.map(f => <FigureCard key={f.id} figure={f} selected={selectedId === f.id} onClick={() => setSelectedId(f.id)} onDoubleClick={() => { setEditingFigure(f); setModalOpen(true); }} />) : filtered.map(f => <div key={f.id} onClick={() => setSelectedId(f.id)} onDoubleClick={() => { setEditingFigure(f); setModalOpen(true); }} className={`${styles.listItem} ${selectedId === f.id ? styles.listItemSelected : ''}`}>{f.name}</div>)}
            ...
            <div className={styles.centerPanel}>
                {selected ? (<>
                    <div className={styles.centerHeader}><div className={styles.centerTitle}>{selected.name}</div><div className={styles.centerMeta}><span>{selected.status}</span>{selected.manufacturer && <span>🏭 {selected.manufacturer}</span>}{selected.scale && <span>📏 {selected.scale}</span>}</div></div>
                    <div style={{ display: 'flex', gap: '8px', padding: '12px 16px', background: 'var(--bg-secondary)', borderBottom: '1px solid var(--border)' }}>
                        <button onClick={() => { setEditingFigure(selected); setModalOpen(true); }} className={styles.editorBtn}>✏️ {$t.edit}</button>
                        <button onClick={async () => { if (confirm($t.deleteFigureConfirm)) { await deleteFigureAPI(selected.id); setSelectedId(null); await loadFigures(); } }} className={styles.editorBtn} style={{ background: 'var(--danger)' }}>🗑 {$t.deleteFigure}</button>
                    </div>
