...
export function initializeComponents(): void {
    ...
    filterBar = new FilterBar(() => renderTable());
...
export async function refreshFilterData(): Promise<void> {
    try {
        ...
        if (paintModalManager) {
            paintModalManager.updateData(brands, baseColors);
        }
    } catch (error) {
...
export async function loadFilterData(): Promise<void> {
...
export async function updateSeriesFilter(paints: Paint[]): Promise<void> {
...
export async function renderTable(): Promise<void> {
    try {
        ...
        tableBody.innerHTML = `<tr><td colspan="9" class="loading">${t().msgErrorLoading}...</td><table>`;
    }
}
...
export function setupResetFiltersButton(): void {
    ...
    if (resetBtn) {
        resetBtn.onclick = () => {
            ...
            if (filterSeriesEl) {
                const t_ = t();
                filterSeriesEl.innerHTML = '<option value="">' + t_.filterAll + '</option>';
            }
